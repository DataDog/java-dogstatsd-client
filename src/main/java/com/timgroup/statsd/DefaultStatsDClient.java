...
public abstract class DefaultStatsDClient implements StatsDClient{
   ...
   * background thread, the caller thread or any other threading model the implementation chooses.
  ...
  private void appendSampleRate(double sampleRate, StringBuilder sb) {
    sb.append("|@");
    // Use of a formatter here so that the sample rate stays the same as when using String.format
  ...
  protected class Sender {
     ...
     * #blockingSend()} and then the message gets added to the buffer.
     *
